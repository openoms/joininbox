...
if [ $# -eq 0 ]||[ "$1" = "-h" ]||[ "$1" = "--help" ];then
  ...
  echo
  echo "install.lnd.sh [selftest|torinstance]"
  echo 
  exit 1
fi
...
if [ "$1" =  menu ]||[ "$1" =  hexstring ]||[ "$1" =  scp ]||\
   ...
   [ "$1" =  thunderhub ]||[ "$1" =  sphinx ]||[ "$1" =  selftest ]||\
   [ "$1" =  torinstance ];then
  ...
  if [ "$1" = "menu" ]; then
      ...
      OPTIONS+=(TEST "Test if reachable over Tor") 
      OPTIONS+=(TOR "Add a Tor instance for lnd${NODENUMBER}") 
...
if [ "$1" =  write-sphinx-environment ];then
  ...
  # TOR SELF TEST #
  #################
  # proxychains4 nmap -p9735 $(lncli1 getinfo|jq -c .uris|cut -d@ -f2|cut -d: -f1) 
  elif [ "${exportType}" = "selftest" ];then
    if [ -f /etc/proxychains.lnd${NODENUMBER}.conf ];then
      CONF="-f /etc/proxychains.lnd${NODENUMBER}.conf"
    else
      CONF=""
    fi
    echo "Running:"
    command=$(echo "proxychains4 $CONF nmap -p9735 \$(sudo -u ${LNDUSER} /usr/local/bin/lncli\
 --lnddir=/home/${LNDUSER}/.lnd${NODENUMBER}/\
 --network=$NETWORK\
...
|jq -c .uris|cut -d@ -f2|cut -d: -f1)")
  ...
  ##########################
  elif [ "${exportType}" = "torinstance" ];then
    NODENAME="lnd${NODENUMBER}"
    SOCKSPORT=9${NODENUMBER}70
    CONTROLPORT=$((SOCKSPORT+1))

    echo "# Creating a dedicated Tor instance for $NODENAME with SocksPort $SOCKSPORT and ControlPort $CONTROLPORT"
...
" | sudo tee /etc/proxychains.$NODENAME.conf
    ...
    # modify LND service
    echo "# Make sure $NODENAME is disabled"
